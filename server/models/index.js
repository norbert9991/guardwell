...
const syncDatabase = async (force = false) => {
    try {
        ...
        if (shouldAlter) {
            ...
            // Manually ensure new Worker columns exist
            try {
                const queryInterface = sequelize.getQueryInterface();
                const workerColumns = await queryInterface.describeTable('workers');

                if (!workerColumns.profileImage) {
                    await queryInterface.addColumn('workers', 'profileImage', {
                        type: require('sequelize').DataTypes.TEXT('long'),
                        allowNull: true
                    });
                    console.log('✅ Added profileImage column to workers');
                }
            } catch (colError) {
                if (!colError.message.includes('doesn\'t exist')) {
